package weihui.bcss.support.dtp.core.queue;
...
/**
 * 支持弹性伸缩的linked queue，本类拷贝之 JDK1.8 LinkedBlockingQueue，在该类基础上做了如下修改
 * 1.去除 capacity 不可变的 final 修饰符,添加 volatile 修饰符保证可见性,有序性。
 * 2.新增 setCapacity() 方法支持重置队列大小，通过 lock 保证setCapacity 的线程安全。
...
public class ResizableCapacityLinkedBlockIngQueue<E> extends AbstractQueue<E>
    ...
    /**
     *  1.去除 不可变的 final 修饰符
     *  2.添加 volatile 修饰符保证可见性,有序性
     *  3.通过 lock 保证capacity 的一致性
     * */
    private volatile int capacity;
...
//     }
    ...
    public void setCapacity(int capacity) {
        ...
        synchronized (count){
            ...
            //重新设置 capacity
            this.capacity = capacity;
            //扩容 && 如果有入队线程进入了notFull Condition阻塞队列，则主动唤醒一次(目的是解决入队 if (c == capacity) 失效，导致无法及时通知到阻塞在notFull中的线程)
            if (this.capacity > oldCapacity && count.get() == oldCapacity) {
                signalNotFull();
            }
        }
    }
    ...
    public E take() throws InterruptedException {
        ...
        //1.只有在队列满了才会触发 notFull.await(); 造成put线程进入 Condition 等待队列
        //2.那就只有在队列满后,触发task，poll消费的那个线程才能去唤醒 notFull.signal() 入队线程
        if (c == capacity)
            //这个值变大后，需要通知 notFull.signal() 唤醒入队线程，否则需要等到下一次 put 才会唤醒已经等待的put入队线程。
            signalNotFull();
        return x;
    }
